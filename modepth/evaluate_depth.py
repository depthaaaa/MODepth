...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.eval_teacher:
            ...
            bins_path = os.path.join(opt.load_weights_folder, "bins.pth")
        ...
        depth_decoder = networks.DepthDecoderUnet(opt.scales, in_channels=[64, 128, 192, 256], bins=opt.dim_out)

        if opt.dim_out:
            if opt.binstype == "adabins":
                bins = networks.AdaBins(in_channels=opt.dim_out, embedding_dim=opt.dim_out // 2, min_val=0.001, max_val=80.0)
            elif opt.binstype == "adaplanes":
                bins = networks.AdaPlanes(in_channels=opt.dim_out, embedding_dim=opt.dim_out // 2, min_val=0.001, max_val=80.0)

        model_dict = encoder.state_dict()
        encoder.load_state_dict({k: v for k, v in encoder_dict.items() if k in model_dict})
        depth_decoder.load_state_dict(torch.load(decoder_path))
        bins.load_state_dict(torch.load(bins_path))
        ...
        # do inference
        with torch.no_grad():
            for i, data in enumerate(tqdm.tqdm(dataloader, ncols=85)):
                ...
                if opt.eval_teacher:
                    ...
                    output = bins(depth_decoder(output))
    ...
    errors = []
    ratios = []
    for i in tqdm.tqdm(range(pred_disps.shape[0]), ncols=85):
