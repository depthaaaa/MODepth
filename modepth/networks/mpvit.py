...
class ResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = Conv2d_BN(in_features,
                               hidden_features,
                               act_layer=act_layer, norm_cfg=norm_cfg)
        self.dwconv = nn.Conv2d(
            hidden_features,
            hidden_features,
            3,
...
class MHCA_stage(nn.Module):
    ...
    def __init__(
        ...
        self.InvRes = ResBlock(in_features=embed_dim, out_features=embed_dim, norm_cfg=norm_cfg)
...
class MPViT(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed_stages = nn.ModuleList([
            Patch_Embed_stage(
                # embed_dims[idx] if idx == 0 else embed_dims[idx - 1],
                embed_dims[idx],
        ...
        self.mhca_stages = nn.ModuleList([
            MHCA_stage(
                # embed_dims[idx - 1] if not idx == 0 else embed_dims[idx],  # [64, 64, 128, 256]
                # embed_dims[idx],  # [128, 256, 512, 512]
                embed_dims[idx],
                embed_dims[idx + 1]
                if not (idx + 1) == self.num_stages
                else embed_dims[idx],
                # if not (idx + 1) == self.num_stages else embed_dims[idx],         # [128, 256, 512, 512]
                num_layers[idx],  # [1, 3, 6, 3]
                num_heads[idx],  # [8, 8, 8, 8]
                mlp_ratios[idx],  # [4, 4, 4, 4]
                num_path[idx],  # [2, 3, 3, 3]
                drop_path_list=dpr[idx],  # [0, 0, 0, 0]
            ) for idx in range(self.num_stages)
        ])
    ...
    def forward_features(self, x):
        ...
        for idx in range(self.num_stages):
            ...
            print("att_inputs", idx, att_inputs[0].shape)
            #outs.append(att_inputs)
            x = self.mhca_stages[idx](att_inputs)
            outs.append(x)
        # x 0 torch.Size([1, 64, 96, 320])
        # att_inputs 0 torch.Size([1, 64, 48, 160])
        # x 1 torch.Size([1, 128, 48, 160])
        # att_inputs 1 torch.Size([1, 128, 24, 80])
        # x 2 torch.Size([1, 192, 24, 80])
        # att_inputs 2 torch.Size([1, 192, 12, 40])
        # x 3 torch.Size([1, 256, 12, 40])
        # att_inputs 3 torch.Size([1, 256, 6, 20])
...
@register_model
def mpvit_xsmall(pretrained, **kwargs):
    """mpvit_xsmall :
    ...
    if pretrained:
        ...
        load_state_dict(model, net_dict, strict=False, logger=logger)
        del net_dict
        del state_dict
        del checkpoint
        del logger
...
if __name__ == "__main__":
    model = mpvit_xsmall(True)
    model.to("cuda")
    # model.eval()
    inputs = torch.randn(1, 3, 192, 640).to("cuda")
    ...
    print("feats[0]={}, feats[1]={}, feats[2]={}, feats[3]={}, feats[4]={}"
          .format(feats[0].shape, feats[1].shape, feats[2].shape, feats[3].shape, feats[4].shape))
    # feats[0] = torch.Size([1, 64, 96, 320])
    # feats[1] = torch.Size([1, 128, 48, 160])
    # feats[2] = torch.Size([1, 192, 24, 80])
    # feats[3] = torch.Size([1, 256, 12, 40])
