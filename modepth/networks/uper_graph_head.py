...
from modepth.graph_utils import resize, normal_init
...
class PPM(nn.ModuleList):
    ...
    def __init__(self, pool_scales, in_channels, channels, conv_cfg, norm_cfg,
                 act_cfg, align_corners, num_groups):
        ...
        for pool_scale in pool_scales:
            ...
            if pool_scale == 1: norm_cfg = dict(type='GN', requires_grad=True, num_groups=num_groups)
            self.append(
                nn.Sequential(
...
class PSP(BaseDecodeHead):
    ...
    def psp_forward(self, inputs):
        ...
        x = inputs[-1]
        psp_outs = [x]
        # print("psp.size={}".format(inputs[-1].size()))
