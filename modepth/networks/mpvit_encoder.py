...
from modepth.layers import BackprojectDepth, Project3D
...
def resnet_multiimage_input(num_layers, pretrained=False, num_input_images=1):
    ...
    if pretrained:
        ...
        conv1w = torch.nn.init.kaiming_normal_(torch.ones(64, num_ch_p_img * num_input_images, 7, 7))
        for l in range(num_input_images):
            conv1w[:, num_ch_p_img*l:num_ch_p_img*l+3, :, :] = loaded['conv1.weight'] / num_input_images
        # torch.cat(
        #     [loaded['conv1.weight']] * num_input_images, 1) / num_input_images
        
        loaded['conv1.weight'] = conv1w

        model.load_state_dict(loaded)
    return model


class MPViTEncoderMatching(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, input_height, input_width,
        ...
        super(MPViTEncoderMatching, self).__init__()

        self.adaptive_bins = adaptive_bins
        self.depth_binning = depth_binning
        self.set_missing_to_max = True

        self.num_ch_enc = np.array([64, 128, 192, 256])
        ...
        encoder = mpvit_xsmall(pretrained=pretrained)
        self.layer0 = encoder.stem
        self.layer1 = nn.Sequential(encoder.patch_embed_stages[0], encoder.mhca_stages[0])
        self.layer2 = nn.Sequential(encoder.patch_embed_stages[1], encoder.mhca_stages[1])
        self.layer3 = nn.Sequential(encoder.patch_embed_stages[2], encoder.mhca_stages[2])
        # self.layer4 = nn.Sequential(encoder.patch_embed_stages[3], encoder.mhca_stages[3])
        ...
        self.compute_depth_bins(min_depth_bin, max_depth_bin)  # cost volume
    ...
    def compute_depth_bins(self, min_depth_bin, max_depth_bin):
        ...
        if self.depth_binning == 'inverse':
            ...
            self.depth_bins = np.linspace(min_depth_bin, max_depth_bin, self.num_depth_bins)  # [0.1, 20] 96
        else:
        ...
        for depth in self.depth_bins:
            ...
            self.warp_depths.append(depth)  # cost volume
        self.warp_depths = torch.stack(self.warp_depths, 0).float()  # [num_depth_bins, height, width]
        if self.is_cuda:
    ...
    def match_features(self, current_feats, lookup_feats, relative_poses, K, invK):
        ...
        for batch_idx in range(len(current_feats)):
            ...
            volume_shape = (self.num_depth_bins, self.matching_height, self.matching_width)  # [96, H/4, W/4]
            ...
            _lookup_feats = lookup_feats[batch_idx:batch_idx + 1]  # 1 x frames=1 x 3 x h x w
            _lookup_poses = relative_poses[batch_idx:batch_idx + 1]

            _K = K[batch_idx:batch_idx + 1]
            _invK = invK[batch_idx:batch_idx + 1]
            
            world_points = self.backprojector(self.warp_depths,
                                              _invK)  # [num_depth_bins, height, width],
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                lookup_feat = lookup_feat.repeat([self.num_depth_bins, 1, 1, 1])  # [96, 3, 48, 160]
                ...
                warped = F.grid_sample(lookup_feat, pix_locs, padding_mode='zeros', mode='bilinear',
                                       align_corners=True)  
                # warped.shape = warped size=torch.Size([96, 64, 48, 160])
                # pix_locs.shape = torch.Size([96, 48, 160, 2]) [-1, 1]
                ...
                # current_feats.shape = [b, C, H/4, W/4]
    ...
    def forward(self, current_image, lookup_images, poses, K, invK,
        ...
        # feature extraction
        self.features = self.feature_extraction(current_image, return_all_feats=True)  # [feats_0, feats_1]
        current_feats = self.features[-1]  # feats_1 [b, C, H/4, W/4]

        # feature extraction on lookup images - disable gradients to save memory
        with torch.no_grad():
            if self.adaptive_bins:
                self.compute_depth_bins(min_depth_bin, max_depth_bin)  # [num_depth_bins, height, width]

            batch_size, num_frames, chns, height, width = lookup_images.shape  # num_frames = 1
            ...
            lookup_feats = self.feature_extraction(lookup_images,
                                                   return_all_feats=False)  # feats_1
...
class ResnetEncoder(nn.Module):
    ...
    def forward(self, input_image):
        ...
        x = self.encoder.bn1(x)  # input torch.Size([10, 64, 192, 640])
        self.features.append(self.encoder.relu(x))  # torch.Size([10, 64, 96, 320])
        self.features.append(self.encoder.layer1(self.encoder.maxpool(self.features[-1])))  # torch.Size([10, 64, 48, 160])
        self.features.append(self.encoder.layer2(self.features[-1]))  # torch.Size([10, 128, 24, 80])
        self.features.append(self.encoder.layer3(self.features[-1]))  # torch.Size([10, 256, 12, 40])
        self.features.append(self.encoder.layer4(self.features[-1]))  # torch.Size([10, 512, 6, 20])
...
if __name__ == '__main__':
    ...
    lookup_images = torch.rand([2, 1, 3, 192, 640])
    ralative_pose = torch.rand([2, 1, 4, 4])
    ...
    # restmp = ResnetEncoder(18, False)
    # feats = restmp(img)
    K = torch.from_numpy(K).unsqueeze(0).repeat([2, 1, 1])
    inv_K = torch.from_numpy(inv_K).unsqueeze(0).repeat([2, 1, 1])
    restmp = MPViTEncoderMatching(18, True, 192, 640, adaptive_bins=True,
                                   min_depth_bin=0.1, max_depth_bin=20.0, depth_binning="linear", num_depth_bins=96)

    feats, _, _ = restmp(img, lookup_images, ralative_pose, K, inv_K, min_depth_bin=0.1, max_depth_bin=20.0)
    ...
    # print(self.features[2].size())
    # print(self.features[3].size())
    # print(self.features[4].size())
    # torch.Size([10, 64, 96, 320])
    # torch.Size([10, 64, 48, 160])
    # torch.Size([10, 128, 24, 80])
    # torch.Size([10, 256, 12, 40])
    # torch.Size([10, 512, 6, 20])

    # feats[0]=torch.Size([2, 64, 96, 320])
    # feats[1]=torch.Size([2, 128, 48, 160])
    # feats[2]=torch.Size([2, 256, 24, 80])
    # feats[3]=torch.Size([2, 512, 12, 40])
